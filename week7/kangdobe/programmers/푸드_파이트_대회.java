...
// food[0] = 무조건 1, 물이다, food[n] = 2% = 1이면 홀수다<<< X , food[n]/2 = 셋팅할 n의 값
// 그럼 for문으로 돌려서 STRING 이니까 (+)로 넣고 완성 후 0넣고 그리고 sb.reverse().toString(); 로 뒤집어서 넣기 하면 될듯?
// (+) 왜 안되는거지 append로 하자 StringBuilder왜 자꾸 고장나는??
public class 푸드_파이트_대회 {
    public String solution(int[] food) {
        StringBuilder set = new StringBuilder();

        for (int i = 1; i < food.length; i++) {
            int cnt = food[i] / 2; // i 번째 음식 수 2로 나누고 소숫점 버림
        ...
        // 아니 이거 왜이렇게 안되
        String setback = new StringBuilder(set).reverse().toString();

        return set + "0" + setback;
    }
}
/*
 ...
 * 이번 대회를 위해 수웅이는 음식을 주문했는데, 대회의 조건을 고려하지 않고 음식을 주문하여 몇 개의 음식은 대회에 사용하지 못하게
 * 되었습니다.
 * 
 ...
 * 
 * 수웅이가 준비한 음식의 양을 칼로리가 적은 순서대로 나타내는 정수 배열 food가 주어졌을 때, 대회를 위한 음식의 배치를 나타내는
